...
pub fn read_race_nm(path: &str) -> Vec<RaceName> {
    ...
    let race_names = (0..NUMBER_OR_RACES).map(|idx| {
        ...
        let long_name = read_koi8r_str(long_names, long_name_index, LONG_NAME_LENGTH);
        let short_name = read_koi8r_str(short_names, short_name_index, SHORT_NAME_LENGTH);
        let adjective = read_koi8r_str(adjectives, adjective_index, ADJECTIVE_LENGTH);
        RaceName {
